...
//   }
  ...
  // (회원가입 타입 선택)
  // chosenRegisterTypeCode : (1 : Email, 2 : Phone, 3 : Google, 4 : Apple, 5 : Naver, 6 : Kakao)
  Future<void> chooseRegisterType(int chosenRegisterTypeCode) async {
    // 회원가입 타입이 결정되었으므로 본인 검증(본인 이메일, 본인 전화번호, 본인 OAuth2 계정) 후 필수 입력 정보 입력 페이지로 이동
    switch (chosenRegisterTypeCode) {
      case 1: // Email
        {
          // 이메일 본인 검증 화면으로 이동
          ...
          all_page_register_email_verification.PageOutputVo? pageOutputVo =
              await _context
                  .pushNamed(all_page_register_email_verification.pageName);
          //  종료 여부 확인
      ...
      case 2: // Phone
        {
          // 전화번호 본인 검증 화면으로 이동
          if (!_context.mounted) return;
          all_page_register_phone_number_verification.PageOutputVo?
              pageOutputVo = await _context.pushNamed(
                  all_page_register_phone_number_verification.pageName);
          //  종료 여부 확인
          if (pageOutputVo != null && pageOutputVo.registerComplete) {
            if (!_context.mounted) return;
            _context.pop();
