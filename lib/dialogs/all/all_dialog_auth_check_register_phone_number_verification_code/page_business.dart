...
//   }
    ...
    if (pageViewModel.codeTextEditController.text == "") {
      ...
      var loadingSpinner = all_dialog_loading_spinner.PageEntrance(
        ...
        var responseVo = await api_main_server
            .getRegisterWithPhoneNumberVerificationCheckAsync(api_main_server
                .GetRegisterWithPhoneNumberVerificationCheckAsyncRequestQueryVo(
                    pageViewModel.pageInputVo.phoneNumber,
  ...
  // (검증 이메일 다시 전송)
  void resendVerificationPhoneNumber() {
    // 입력값 검증 완료
    // (로딩 스피너 다이얼로그 호출)
    var loadingSpinner = all_dialog_loading_spinner.PageEntrance(
        all_dialog_loading_spinner.PageInputVo(), (pageBusiness) async {
        ...
        if (networkResponseObjectOk.responseStatusCode == 200) {
          ...
          await showDialog(
              ...
              builder: (context) => all_dialog_info.PageEntrance(
                  all_dialog_info.PageInputVo(
                      "SMS Resend",
                      "${pageViewModel.pageInputVo.phoneNumber}\nVerification text has been resent.",
                      "Check"),
                  (pageBusiness) {}));
          pageViewModel.codeTextEditErrorMsg = null;
          pageViewModel.codeTextEditController.text = "";
