...
// 페이지 onCreate 시 정보 확인 후, 첫 실행이라면 false 저장
...
class SharedPreferenceWrapper {
  ...
  static const String globalKeyName = "spw_program_start_first_time";
  ...
  static SharedPreferenceWrapperVo? get() {
    ...
    if (savedJsonString.trim() == "") {
      ...
      try {
        ...
        var resultObject = SharedPreferenceWrapperVo(map["value"]);
        semaphore.release();
        return resultObject;
      } catch (e) {
  ...
  static void set(SharedPreferenceWrapperVo? value) {
    ...
    if (value == null) {
      ...
      Map<String, dynamic> map = {"value": value.value};
...
class SharedPreferenceWrapperVo {
  bool value; // 프로그램이 처음 실행되었는지 여부

  SharedPreferenceWrapperVo(this.value);
}
