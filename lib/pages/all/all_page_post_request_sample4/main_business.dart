...
class MainBusiness {
  ...
  final GlobalKey<gw_sfw_wrapper.SfwRefreshWrapperState> pickFile1AreaGk =
      GlobalKey();
  late BuildContext pickFile1AreaContext;
  PlatformFile? pickedFile1;

  final GlobalKey<gw_sfw_wrapper.SfwRefreshWrapperState> pickFile2AreaGk =
      GlobalKey();
  late BuildContext pickFile2AreaContext;
    ...
    showDialog(
        ...
        builder: (context) => all_dialog_loading_spinner.MainWidget(
              ...
              inputVo:
                ...
                Map<String, dynamic> requestBodyMap = {
                  ...
                  "requestFormStringNullable":
                      (input2TextFieldController.text == "")
                          ? null
                          : input2TextFieldController.text,
                  "requestFormInt": input3Text,
                  "requestFormIntNullable":
                      (input4TextFieldController.text == "")
                          ? null
                          : input4TextFieldController.text,
                  "requestFormDouble": input5Text,
                ...
                MultipartFile pickFile1Param = ((kIsWeb)
                    ? MultipartFile.fromBytes(pickedFile1!.bytes!,
                        filename: pickedFile1!.name)
                    : MultipartFile.fromFileSync(pickedFile1!.path!));

                MultipartFile? pickFile2Param = (pickedFile2 == null)
                    ? null
                    : ((kIsWeb)
                        ? MultipartFile.fromBytes(pickedFile2!.bytes!,
                            filename: pickedFile2!.name)
                        : MultipartFile.fromFileSync(pickedFile2!.path!));
                    ...
                    .postService1TkV1RequestTestPostRequestMultipartFormDataJsonAsync(
                        requestBodyVo: api_main_server
                            .PostService1TkV1RequestTestPostRequestMultipartFormDataJsonAsyncRequestBodyVo(
                                ...
                                multipartFile: pickFile1Param,
                                multipartFileNullable: pickFile2Param));
  ...
  void pickFile1() {
    FilePicker.platform.pickFiles().then((FilePickerResult? result) {
      if (result != null && result.files.isNotEmpty) {
        ...
        pickedFile1 = file;
        pickFile1AreaGk.currentState?.refreshUi();
      }
    });
  }
  ...
  void pickFile2() {
    FilePicker.platform.pickFiles().then((FilePickerResult? result) {
      if (result != null && result.files.isNotEmpty) {
        PlatformFile file = result.files.first;
        pickedFile2 = file;
        pickFile2AreaGk.currentState?.refreshUi();
      }
    });
  }
